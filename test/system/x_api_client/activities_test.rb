require "application_system_test_case"

module XApiClient
  class ActivitiesTest < ApplicationSystemTestCase
    setup do
      @activity = x_api_client_activities(:one)
    end

    test "visiting the index" do
      visit activities_url
      assert_selector "h1", text: "Activities"
    end

    test "creating a Activity" do
      visit activities_url
      click_on "New Activity"

      fill_in "X actor", with: @activity.x_actor
      fill_in "X context", with: @activity.x_context
      fill_in "X object", with: @activity.x_object
      fill_in "X result", with: @activity.x_result
      fill_in "X verb", with: @activity.x_verb
      click_on "Create Activity"

      assert_text "Activity was successfully created"
      click_on "Back"
    end

    test "updating a Activity" do
      visit activities_url
      click_on "Edit", match: :first

      fill_in "X actor", with: @activity.x_actor
      fill_in "X context", with: @activity.x_context
      fill_in "X object", with: @activity.x_object
      fill_in "X result", with: @activity.x_result
      fill_in "X verb", with: @activity.x_verb
      click_on "Update Activity"

      assert_text "Activity was successfully updated"
      click_on "Back"
    end

    test "destroying a Activity" do
      visit activities_url
      page.accept_confirm do
        click_on "Destroy", match: :first
      end

      assert_text "Activity was successfully destroyed"
    end
  end
end
